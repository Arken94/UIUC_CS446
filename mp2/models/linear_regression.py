...
class LinearRegression(LinearModel):
    ...
    def backward(self, f, y):
        ...
        Returns:
            total_grad(numpy.ndarray): Gradient of L w.r.t to self.w,
              dimension (ndims+1,1).
        """
        total_grad = self.x.T.dot(f - y) + self.w_decay_factor * self.w
    ...
    def total_loss(self, f, y):
        ...
        Args:
            ...
            y(numpy.ndarray): Ground truth label, dimension (N,1).
        Returns:
            total_loss (float): sum square loss + regularization.
        """
        total_loss = 0.5 * (((y - f) ** 2.).sum() + self.w_decay_factor * np.linalg.norm(self.w) ** 2.)
        # print('L', total_loss / 1e10)
