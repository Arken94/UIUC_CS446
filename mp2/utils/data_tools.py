...
def preprocess_data(dataset,
    ...
    Args:
        ...
        squared_features(bool): Whether to square the features.

    Returns:
        processed_dataset(list): List of numpy arrays x, y.
    ...
    def preprocess_row(row):
        ...
        for col in feature_columns:
            # make it fool proof.
            # Do not include SalePrice as a feature
            # since it is what we are trying to predict
            if col == 'SalePrice':
                continue

            if col == 'BldgType':
                result = result + one_hot_bldg_type(row[columns_to_id[col]])
            else:
                if squared_features:
                    result.append(float(row[columns_to_id[col]]) ** 2)
                else:
                    result.append(float(row[columns_to_id[col]]))
        return result
    ...
    for _, row in dataset.items():
        x.append(preprocess_row(row))
        y.append(float(row[columns_to_id['SalePrice']]))
