...
def createNetwork():
    ...
    # h_conv1 = tf.Print(h_conv1, [h_conv1], 'h_conv1 = ', summarize=20)

    # print(h_conv1.shape)

    h_pool1 = max_pool_2x2(h_conv1)
    # h_pool1 = tf.Print(h_pool1, [h_pool1], 'h_pool1 = ', summarize=20)

    # print(h_pool1.shape)

    h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2, 2) + b_conv2)
    # h_conv2 = tf.Print(h_conv2, [h_conv2], 'h_conv2 = ', summarize=20)
...
def scale_down_epsilon(epsilon, t):
    ...
    # During the observation phase, epsilon is set to 1
    if t < OBSERVE:
        return epsilon

    # scale down after observation phase
...
def run_selected_action(a_t, s_t, game_state):
    ...
    """
    frame, r_t, terminal = game_state.frame_step(a_t)
    frame = cv2.cvtColor(cv2.resize(frame, (80, 80)), cv2.COLOR_BGR2GRAY)
    ret, frame = cv2.threshold(frame, 1, 255, cv2.THRESH_BINARY)
    s_t1 = np.stack((s_t[:,:,1], s_t[:,:,2], s_t[:,:,3], frame), axis=2)
    # np.append((s_t[:,:,1:], frame.reshape(frame.shape[0], frame.shape[1], 1)))
...
def compute_target_q(r_batch, readout_j1_batch, terminal_batch):
    ...
    for i in range(0, len(terminal_batch)):
        ...
        if terminal_batch[i]:
            # the Q-value is only equal to the reward.
            target_q_batch.append(r_batch[i])
        else:
            new_q = r_batch[i] + GAMMA * np.max(readout_j1_batch[i])
            target_q_batch.append(new_q)
...
def trainNetwork(s, readout, sess):
    ...
    Args:
        ...
        readout: the Q value for each possible action in the current state.
        sess: session
    """
