...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_integer('max_iter', 10, 'Number of EM steps to run.')
flags.DEFINE_integer('n_components', 2, 'Number of components')
...
def main(_):
    ...
    """
    # Load dataset.
    unlabeled_data, _ = io_tools.read_dataset('data/simple_test.csv')
    ...
    # Supervised training.
    train_data, train_label = io_tools.read_dataset('data/simple_test.csv')
    model.supervised_fit(train_data, train_label)

    # Eval model.
    eval_data, eval_label = io_tools.read_dataset('data/simple_tesT.csv')
