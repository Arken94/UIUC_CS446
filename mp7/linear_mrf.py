...
class LinearMRF(object):
    ...
    def get_unary_features(self, img):
        ...
        #######################
        return np.column_stack((img == 0, img == 1))
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        '''
        gt = self.beliefs2img(img_features).reshape(img_features.shape[0], 1)
        obj = 0
        #######################
        #IMPLEMENT THIS METHOD#
        #######################
        n_samples = len(unary_beliefs)
        i, j = zip(*self.pairs)

        y_ij_idx = 2 * gt[i,:] + gt[j,:]
        pw_idx = [list(x) for x in enumerate(y_ij_idx)]
        for k in range(n_samples):
            obj += tf.reduce_sum(unary_beliefs[k] * unary_potentials[k])
            obj += tf.reduce_sum(pair_beliefs[k] * pairwise_potentials)

            F = tf.reduce_sum(unary_potentials[k] * img_features) + \
                tf.reduce_sum(tf.gather_nd(pairwise_potentials, pw_idx))
            obj -= F
        return obj
    ...
    def beliefs2img(self, unary_belief):
        ...
        '''
        result = np.empty(len(unary_belief), dtype=np.int32)
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        nodes = np.random.permutation(list(self.neighbors.keys()))
        assignments = [0, 1]
        for node in nodes:
            scores = [self.calculate_local_score(node, assignment,
                                                 unary_beliefs,
                                                 unary_pots,
                                                 pairwise_pots) for
                      assignment in assignments]
            y_i = np.argmax(scores)
            unary_beliefs[node, y_i] = 1
            unary_beliefs[node, (y_i + 1) % 2] = 0
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        score = unary_potentials[i, y_i]

        for j in self.neighbors[i]:
            y_j = np.argmax(unary_beliefs[j,:])
            if i < j:
                col = y_i * 2 + y_j
            else:
                col = y_j * 2 + y_i
            score += pairwise_potentials[self.pair_inds[(i, j)], col]
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        changed = np.count_nonzero(new_unary_beliefs ^ old_unary_beliefs)
        return changed / new_unary_beliefs.size <= convergence_margin
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        #######################
        i, j = zip(*self.pairs)
        return unary_beliefs[i, :][:, [0, 0, 1, 1]] * \
            unary_beliefs[j, :][:, [0, 1, 0, 1]]
