...
class VariationalAutoencoder(object):
    ...
    def _sample_z(self, z_mean, z_log_var):
        ...
        z = None
        ####### Implementation Here ######
        eps = tf.random_normal(tf.shape(z_mean), name='epsilon')
    ...
    def _encoder(self, x):
        ...
        z_log_var = None
        ####### Implementation Here ######
        z_layer_0 = tf.contrib.slim.fully_connected(x, 100, activation_fn=tf.nn.softplus)
        z_layer_1 = tf.contrib.slim.fully_connected(z_layer_0, 50, activation_fn=tf.nn.softplus)
        z_mean = tf.contrib.slim.fully_connected(z_layer_1, self._nlatent, activation_fn=None)
        z_log_var = tf.contrib.slim.fully_connected(z_layer_1, self._nlatent, activation_fn=None)
    ...
    def _decoder(self, z):
        ...
        f = None
        ####### Implementation Here ######
        layer_0 = tf.contrib.slim.fully_connected(z, 50, activation_fn=tf.nn.softplus)
        layer_1 = tf.contrib.slim.fully_connected(layer_0, 100, activation_fn=tf.nn.softplus)
        f = tf.contrib.slim.fully_connected(layer_1, self._ndims, activation_fn=tf.nn.sigmoid)
        return f
    ...
    def _latent_loss(self, z_mean, z_log_var):
        ...
        latent_loss = None
        ####### Implementation Here ######
        latent_loss = 0.5 * tf.reduce_sum(-z_log_var - 1.0 + tf.exp(z_log_var) + tf.square(z_mean) + np.finfo(float).eps, 1)
    ...
    def _reconstruction_loss(self, f, x_gt):
        ...
        recon_loss = None
        ####### Implementation Here ######
        return tf.reduce_mean(tf.reduce_sum(tf.square(x_gt - f), 1))
        #return -tf.reduce_mean(tf.reduce_sum(-0.5 * tf.square(x_gt - f) + tf.log((2 * np.pi) ** -0.5), 1))
    ...
    def loss(self, f, x_gt, z_mean, z_log_var):
        ...
        total_loss = None
        ####### Implementation Here ######
        D_KL = self._latent_loss(z_mean, z_log_var)
        reconstruction = self._reconstruction_loss(f, x_gt)
